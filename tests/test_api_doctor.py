...
class TestApiDoctorCRUD():
    ...
    def test_api_create_doctor(self):
        responce = app.test_client().post('/api/new_doctor',data=self.test_data)
    ...
    def test_api_receive_doctor_by_email(self):
        responce = app.test_client().get('/api/doctor/?id=test@email.com')
        assert responce.status_code == 200  
    ...
    def test_api_update_doctor_not_existing(self):
        ...
        assert responce.status_code == 409
        assert b'Error. No record with such email' in responce.data
